...
class DiarizationService:
    ...
    def __init__(self, provider: str = "deepgram", groq_api_key: str = None):
        ...
        self.groq = GroqTranscriptionClient(self.groq_api_key) if self.groq_api_key else None
        ...
        logger.info(f"DiarizationService initialized (provider={provider}, enabled={self.enabled})")
    ...
    async def transcribe_with_whisper(self, audio_data: bytes) -> List[Dict]:
        ...
        logger.info(f"✅ Gold transcription complete: {len(result.get('segments', []))} segments")
    ...
    async def diarize_meeting(
        ...
        try:
            logger.info(f"🎯 Starting diarization for meeting {meeting_id} with {provider}")
            ...
            logger.info(f"📦 Audio prepared: {audio_duration_seconds:.1f}s, {len(wav_data)} bytes")
    ...
    async def _diarize_with_deepgram(self, audio_data: bytes, meeting_id: str) -> List[SpeakerSegment]:
        """
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            ...
            words = result.get("results", {}).get("channels", [{}])[0].get(
                "alternatives", [{}]
            )[0].get("words", [])
            
            if not words and not utterances:
                logger.warning(f"No results returned by Deepgram for meeting {meeting_id}")
                return []
            ...
            if utterances:
                ...
                for w in words:
                    ...
                    if speaker != current_speaker:
                        if current_segment: raw_segments.append(current_segment)
                        current_speaker = speaker
                        current_segment = SpeakerSegment(
                            speaker=speaker, start_time=w.get("start", 0), end_time=w.get("end", 0),
                            text=w.get("word", ""), confidence=w.get("speaker_confidence", 1.0), word_count=1
                        )
                    else:
                        current_segment.end_time = w.get("end", current_segment.end_time)
            ...
            segments.append(current)
            logger.info(f"Reconstructed {len(segments)} natural segments for {meeting_id}")
            return segments
            
            logger.info(f"Deepgram returned {len(segments)} speaker segments")
            return segments
    
    async def _diarize_with_assemblyai(self, audio_data: bytes, meeting_id: str) -> List[SpeakerSegment]:
        """
        ...
        async with httpx.AsyncClient(timeout=600.0) as client:
            ...
            if upload_response.status_code != 200:
                raise Exception(f"AssemblyAI upload failed: {upload_response.status_code}")
            ...
            if transcript_response.status_code != 200:
                raise Exception(f"AssemblyAI transcription request failed: {transcript_response.status_code}")
            ...
            for utterance in utterances:
                segment = SpeakerSegment(
                    ...
                    start_time=utterance.get("start", 0) / 1000,  # AssemblyAI uses milliseconds
    ...
    async def align_with_transcripts(
        ...
        if diarization_result.status != "completed":
            logger.warning(f"Cannot align - diarization status: {diarization_result.status}")
            return transcripts, {'error': f'Diarization status: {diarization_result.status}'}
        ...
        if not speaker_segments:
            ...
            unknown_transcripts = [
                {**t, "speaker": "Unknown", "text": t.get("text", t.get("transcript", "")),
                 "alignment_state": "UNKNOWN_SPEAKER", "speaker_confidence": 0.0}
