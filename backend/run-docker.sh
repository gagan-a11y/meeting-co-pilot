...
show_log_exit_options() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "whisper-server\|meeting-copilot-backend"; then
...
find_existing_databases() {
    local found_dbs=()
    local default_db_path="/opt/homebrew/Cellar/meeting-copilot-backend/0.0.4/backend/meeting_minutes.db"
    ...
    local common_paths=(
        "/opt/homebrew/Cellar/meeting-copilot-backend/*/backend/meeting_minutes.db"
...
start_server() {
    ...
    if docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "meeting-copilot-backend:"; then
...
show_logs() {
    ...
    case "$service" in
        "whisper")
            ...
            ;;
        "app"|"backend")
            log_cmd+=("meeting-copilot-backend")
            ;;
        "")
...
show_status() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "meeting-copilot-backend"; then
        app_running=true
        local app_port=$(docker port meeting-copilot-backend 5167/tcp 2>/dev/null | cut -d: -f2)
        if [ -n "$app_port" ]; then
            log_info "Meeting App: http://localhost:$app_port"
            # Test connectivity
